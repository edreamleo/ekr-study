#@+leo-ver=5-thin
#@+node:ekr.20241212100516.64: * @file C:\Users\Dev\EKR-Study\python\CODE_PYTHON\CH08\CH08_SEC07_1_LQR.py
#@+others
#@+node:ekr.20241212100516.66: ** import numpy as np
import numpy as np
...
#@+node:ekr.20241212100516.67: ** m = 1
m = 1
M = 5
L = 2
g = -10
...
#@+node:ekr.20241212100516.68: ** # Design LQR Controller
...
#@+node:ekr.20241212100516.69: ** # ODE RHS Function Definition
...
#@+node:ekr.20241212100516.70: ** # Simulate closed-loop system
...
#@+node:ekr.20241212100516.71: ** for k in range(np.floor(len(t)/100)):
for k in range(np.floor(len(t) / 100)):
    plt.plot

#@+node:ekr.20241212100516.72: ** fig,ax = plt.subplots()
fig, ax = plt.subplots()
H = 0.5 * np.sqrt(M / 5)
...
#@+node:ekr.20241212100516.73: ** plot_labels = ('x','v','theta','omega')
...
#@+node:ekr.20241212100516.74: ** # Compare with many examples of Pole
...
#@+node:ekr.20241212100516.75: ** # Plots
## Plots
for count in range(100):
...
#@+node:ekr.20241212100516.76: ** for count in range(100):
for count in range(100):
